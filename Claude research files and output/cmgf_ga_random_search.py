...
class ReversibleCircuit:
    ...
    def __init__(self, n: int, N: int, circuit_name: str = "RC"):
        """
        Initialize the Reversible Circuit
        
        Args:
            n: Number of input lines
            N: Number of gates/levels
            circuit_name: Name of the circuit
        """
        self.n = n  # Number of input lines
        self.N = N  # Number of gates
        self.circuit_name = circuit_name
        self.max_vectors = 2 ** n
        
        # Define gate operations (simplified - you can customize based on your circuit)
        # Each gate is represented as a function that transforms the input
        self.gates = self._initialize_gates()
    
    def _initialize_gates(self) -> List:
        """Initialize gate operations (placeholder - customize for your circuit)"""
        # This is a simplified version. In practice, you'd define specific gate operations
        # For example: Toffoli gates, CNOT gates, etc.
        gates = []
        for i in range(self.N):
            # Each gate can be a function that transforms the state
            # For demonstration, we'll use simple XOR-based operations
            gates.append(lambda x, idx=i: x ^ (1 << (idx % self.n)))
        return gates
    
    def compute_output(self, input_vector: int, skip_gate: int = -1) -> int:
        """
        Compute circuit output for a given input
        
        Args:
            input_vector: Input as decimal number
            skip_gate: Gate to skip (for CMGF simulation), -1 for fault-free
...
class CMGFGeneticAlgorithm:
    """Genetic Algorithm for CMGF Test Generation using Random Search"""
    
    def __init__(self, circuit: ReversibleCircuit, threshold: float = 100.0,
                 population_size: int = None, max_generations: int = 100):
        """
        Initialize the GA
    ...
    def stage_ii_random_selection(self, size: int = None) -> List[int]:
        ...
        available_vectors = list(range(self.circuit.max_vectors))
        selected = random.sample(available_vectors, min(size, len(available_vectors)))
        
        return selected
    
    def stage_iii_fitness_computation(self, vector: int) -> Tuple[float, Set[int]]:
        """
        ...
        fault_free_output = self.circuit.compute_output(vector, skip_gate=-1)
    ...
    def stage_v_crossover(self, parent_pairs: List[Tuple[int, int]]) -> List[int]:
        ...
        for parent1, parent2 in parent_pairs:
            # Convert to binary
            bin1 = format(parent1, f'0{self.circuit.n}b')
            bin2 = format(parent2, f'0{self.circuit.n}b')
            
            # Random crossover point
            crossover_point = random.randint(1, self.circuit.n - 1)
    ...
    def stage_vi_mutation(self, children: List[int]) -> List[int]:
        ...
        for child in children:
            if random.random() < mutation_prob:
                # Convert to binary
                child_bin = list(format(child, f'0{self.circuit.n}b'))
                
                # Flip a random bit
                flip_position = random.randint(0, self.circuit.n - 1)
                child_bin[flip_position] = '0' if child_bin[flip_position] == '1' else '1'
                
                # Convert back to decimal
                mutated_child = int(''.join(child_bin), 2)
    ...
    def stage_viii_minimal_test_set(self, fin_pop: List[Tuple[int, float, Set[int]]], 
                                    L: int = None) -> Tuple[List[int], float]:
        """
    ...
    def compute_combined_coverage(self, test_set: List[int]) -> Tuple[float, Set[int]]:
        ...
        coverage = (len(all_detected) / self.circuit.N) * 100 if self.circuit.N > 0 else 0
        return coverage, all_detected
    
    def find_best_combination(self, vectors: List[int]) -> Tuple[List[int], float]:
        """Find the best combination of vectors for maximum coverage"""
        best_combo = []
        best_coverage = 0.0
        
        for size in range(1, min(len(vectors) + 1, self.circuit.n + 1)):
    ...
    def run(self, verbose: bool = True) -> Tuple[List[int], float]:
        ...
        if verbose:
            print(f"=== CMGF GA with Random Search ===")
            print(f"Circuit: {self.circuit.circuit_name}")
            print(f"Input lines (n): {self.circuit.n}")
            print(f"Gates (N): {self.circuit.N}")
        ...
        if verbose:
            ...
            print(f"Best Test Set: {test_set}")
            print(f"Best Coverage: {coverage:.2f}%")
        
        return test_set, coverage


def main():
    """Example usage of the CMGF GA algorithm"""
    
    # Example 1: Circuit 3_17tc (n=3, N=17)
    print("Example 1: Circuit 3_17tc")
    print("-" * 50)
    circuit1 = ReversibleCircuit(n=3, N=17, circuit_name="3_17tc")
    ga1 = CMGFGeneticAlgorithm(circuit1, threshold=100.0, population_size=3, max_generations=50)
    test_set1, coverage1 = ga1.run(verbose=True)
    
    print("\n" + "=" * 50 + "\n")
    
    # Example 2: Smaller circuit (n=4, N=10)
    print("Example 2: Custom Circuit (n=4, N=10)")
    print("-" * 50)
    circuit2 = ReversibleCircuit(n=4, N=10, circuit_name="custom_4_10")
    ga2 = CMGFGeneticAlgorithm(circuit2, threshold=100.0, population_size=4, max_generations=30)
    test_set2, coverage2 = ga2.run(verbose=True)
    
    # Convert test sets to binary for better visualization
    print("\n" + "=" * 50)
    print("\nFINAL RESULTS:")
    print(f"\nCircuit 3_17tc:")
    print(f"  Test Set (Decimal): {test_set1}")
    print(f"  Test Set (Binary): {[format(v, '03b') for v in test_set1]}")
    print(f"  Coverage: {coverage1:.2f}%")
    
    print(f"\nCircuit custom_4_10:")
    print(f"  Test Set (Decimal): {test_set2}")
    print(f"  Test Set (Binary): {[format(v, '04b') for v in test_set2]}")
